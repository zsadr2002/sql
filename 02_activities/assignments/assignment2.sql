...
SELECT customer_id, 
  ...
  FROM 
		...
		FROM customer_purchases) AS most_recent_visit 
	where label = 1;
...
VALUES (40, 'orange', 4, 1, 1, CURRENT_TIMESTAMP);
...
AND snapshot_timestamp = (SELECT MIN(snapshot_timestamp) FROM product_units WHERE product_name = 'orange');
